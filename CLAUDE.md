...
3. **Run migrations**: `db/migration_001_price_changes_unique.md`, `db/migration_002_daily_compliance_rollup.md`, `db/migration_003_stations_in_bbox.md` (the bbox RPC the frontend calls on `moveend`), `db/migration_004_compliance_stats_by_date.md` (original per-day rollup RPC), `db/migration_005_compliance_stats_history.md` (introduces the `compliance_stats_history` aggregate table, rewrites the stats RPC to read from it, and adds the history upsert into `recompute_daily_compliance`).
